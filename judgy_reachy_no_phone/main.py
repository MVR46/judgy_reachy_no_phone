...
def _get_status_value(status, key: str, default=None):
    """Read SDK status fields from either dicts or typed status objects."""
...
class JudgyReachyNoPhone(ReachyMiniApp):
    ...
    def _store_latest_frame(self, frame):
        """Store the latest camera frame without blocking camera capture."""
        with self._frame_lock:
            self.latest_frame = frame.copy()

    def _get_latest_frame(self):
        """Return a copy of the latest camera frame for background detection."""
        with self._frame_lock:
            if self.latest_frame is None:
                return None
            return self.latest_frame.copy()

    def _enqueue_detection_event(self, event):
        """Queue a detection event for the main robot loop."""
        if event:
            with self._lock:
                self.detection_event_queue.append(event)

    def _pop_detection_event(self):
        """Pop one detection event, if any."""
        with self._lock:
            if not self.detection_event_queue:
                return None
            return self.detection_event_queue.pop(0)

    def _has_detection_events(self):
        """Check whether a detection event is waiting."""
        with self._lock:
            return bool(self.detection_event_queue)

    def _reset_person_centering_observation(self):
        """Forget observed offset without changing the robot's held pose."""
        self.person_centering_target = None
        self.person_centering_stable_frames = 0

    def _center_on_person(self, reachy: ReachyMini):
        """Recenter in brief smooth moves, then hold still for vision stability."""
        if not self.config.PERSON_CENTERING_ENABLED:
            return

        now = time.time()
        if now < self.person_centering_pause_until:
            return

        if now - self.last_person_centering_time < self.config.PERSON_CENTERING_INTERVAL:
            return
        ...
        with self._detector_lock:
            target = self.detector.get_person_target(frame.shape)
        if target is None:
            self._reset_person_centering_observation()
            return

        smoothing = float(np.clip(self.config.PERSON_CENTERING_SMOOTHING, 0.0, 1.0))
        if self.person_centering_target is None:
            self.person_centering_target = {
                "x_offset": float(target["x_offset"]),
                "y_offset": float(target["y_offset"]),
            }
        else:
            self.person_centering_target["x_offset"] = (
                (1.0 - smoothing) * self.person_centering_target["x_offset"]
                + smoothing * float(target["x_offset"])
            )
            self.person_centering_target["y_offset"] = (
                (1.0 - smoothing) * self.person_centering_target["y_offset"]
                + smoothing * float(target["y_offset"])
            )
        ...
        if drift < self.config.PERSON_CENTERING_RECENTER_THRESHOLD:
            return

        self.person_centering_stable_frames += 1
        if self.person_centering_stable_frames < self.config.PERSON_CENTERING_STABLE_FRAMES:
            self.last_person_centering_time = now
            return

        max_yaw = float(np.deg2rad(self.config.PERSON_CENTERING_MAX_YAW_DEG))
        max_step = float(np.deg2rad(self.config.PERSON_CENTERING_MAX_YAW_STEP_DEG))
        yaw_step = float(np.clip(
            -x_offset * self.config.PERSON_CENTERING_YAW_GAIN,
            -max_step,
            max_step,
        ))
        self.person_centering_yaw = float(np.clip(
            self.person_centering_yaw + yaw_step,
            -max_yaw,
            max_yaw,
        ))
        ...
        reachy.goto_target(
            head=head,
            body_yaw=self.person_centering_yaw,
            duration=self.config.PERSON_CENTERING_MOVE_DURATION,
            method="minjerk",
        )
        self.last_person_centering_time = now
        self.person_centering_pause_until = (
            time.time()
            + self.config.PERSON_CENTERING_MOVE_DURATION
            + self.config.PERSON_CENTERING_HOLD_SECONDS
        )
        self._reset_person_centering_observation()

    def _pause_person_centering(self, seconds: float = 1.0):
        """Let blocking animations finish before the live centering loop resumes."""
        self.person_centering_pause_until = time.time() + seconds
        self._reset_person_centering_observation()
    ...
    def _camera_thread(self, webcam, stop_event: threading.Event):
        ...
        try:
            while not stop_event.is_set() and self.camera_running:
                ...
                # Store frame for background detection
    ...
    def _detection_thread(self, stop_event: threading.Event):
        """Run YOLO separately so slow inference cannot freeze the camera UI."""
        fps_counter = 0
        fps_start = time.time()
        ...
        try:
            while not stop_event.is_set():
                ...
                try:
                    ...
                    self._enqueue_detection_event(event)

                    fps_counter += 1
                    if time.time() - fps_start >= 1.0:
                        self.detection_fps = fps_counter
                        fps_counter = 0
                        fps_start = time.time()
                except Exception as e:
                ...
                # Keep the CM4 responsive; the camera thread still updates
                # the preview while this lower-rate detector works.
                time.sleep(0.2)
    ...
    def run(self, reachy_mini: ReachyMini, stop_event: threading.Event):
        ...
        # Auto-detect camera source. When running this app off-robot on a Mac,
        # use the Mac webcam so YOLO does not depend on remote WebRTC video.
        ...
        if use_local_webcam:
            logger.info("Using local webcam for detection...")
            self.camera_loading_status = "connecting"
            self.camera_loading_message = "Opening local webcam..."

            webcam = cv2.VideoCapture(0)
            if not webcam.isOpened():
                logger.error("Failed to open local webcam!")
                self.camera_loading_status = "error"
                self.camera_loading_message = "Failed to open webcam"
                webcam = None
            else:
                logger.info("Local webcam opened successfully!")
        ...
        try:
            while not stop_event.is_set():
                ...
                # Process detection events from background detection thread
                while True:
                    event = self._pop_detection_event()
                    if event is None:
                        break
                    try:
                        if event == "picked_up":
                            self._pause_person_centering(2.0)
                            self._handle_phone_pickup(reachy_mini)
                            self._pause_person_centering(1.0)
                        elif event == "put_down" and self.praise_enabled:
                            self._pause_person_centering(1.5)
                ...
                if self.is_monitoring and not self._has_detection_events():
                    try:
                        self._center_on_person(reachy_mini)
                    except Exception as e:
                        logger.debug(f"Person centering error: {e}")

                if breath_counter >= BREATH_INTERVAL:
                    breath_counter = 0
                    # Only do idle breathing if no events pending (to avoid blocking)
                    if self.is_monitoring and not self.detector.phone_visible and not self._has_detection_events():
                        try:
                            # Pass callback to check for events during breathing
                            self._pause_person_centering(1.0)
                            idle_breathing(reachy_mini, should_stop=self._has_detection_events)
                            self._pause_person_centering(0.5)
                        except:
                            pass
    ...
    def _handle_phone_pickup(self, reachy: ReachyMini):
        ...
        # Pure Reachy mode uses the existing recorded emotion presets.
        if self.llm.personality == "pure_reachy" and self.emotions:
            ...
            logger.info(f"Pure Reachy preset shame: {emotion_name}")
            reachy.play_move(emotion)
            return

        # Other personalities use the existing prewritten preset lines when no
        # LLM key is configured, then the existing offense-count animation.
        try:
            text = self.llm.get_response(count)
            logger.info(f"Preset personality shame ({self.llm.personality}): {text}")
            ...
            logger.error(f"Preset personality speech failed: {e}")

        try:
            animation = get_animation_for_count(count)
            animation(reachy)
        except Exception as e:
            logger.error(f"Preset personality animation failed: {e}")
    ...
    def _run_ui(self, reachy_mini: ReachyMini, stop_event: threading.Event):
        ...
        def toggle_monitoring(req: ToggleRequest):
            if self.is_monitoring:
                ...
                self.person_centering_pause_until = 0.0
                self.person_centering_stable_frames = 0
                self.last_person_centering_time = 0.0
