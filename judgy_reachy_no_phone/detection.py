        ...
        try:
            ...
            for result in results:
                ...
                for box in result.boxes:
                    cls = int(box.cls)
                    if cls not in (self.PERSON_CLASS_ID, self.PHONE_CLASS_ID):
                        continue

                    conf = float(box.conf)
                    x1, y1, x2, y2 = map(int, box.xyxy[0])
                    track_id = int(box.id[0]) if box.id is not None else None
                    class_name = "person" if cls == self.PERSON_CLASS_ID else "cell phone"
        ...
        try:
            for result in self.last_detections:
                for box in result.boxes:
                    ...
                    class_name = self.yolo_model.names[cls] if self.yolo_model else "object"

                    color = (0, 255, 0) if cls == self.PHONE_CLASS_ID else (255, 180, 0)
